...
# Database setup
DATABASE_URL = "postgresql://postgres:1@localhost:5432/QA97.2"
...
def test_add_subject():
    session = Session()
    subject = Subject(subject_id=100, subject_title="History")
    session.add(subject)
    session.commit()

    saved = session.query(Subject).filter_by(subject_id=100).first()
    assert saved.subject_title == "History"
...
def test_update_subject():
    session = Session()
    subject = Subject(subject_id=200, subject_title="Chemistry")
    session.add(subject)
    session.commit()

    subject.subject_title = "Advanced Chemistry"
    session.commit()

    updated = session.query(Subject).filter_by(subject_id=200).first()
    assert updated.subject_title == "Advanced Chemistry"
